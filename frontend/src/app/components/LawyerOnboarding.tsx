...
export default function LawyerOnboarding({ dbUser, initialStep, onComplete }: { dbUser: any, initialStep: 1 | 2 | 3, onComplete: () => void }) {
  ...
  const handleSaveBio = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://justice-pal-cjhn.vercel.app"}/api/lawyers/profile`, {
        method: "PUT",
        headers: {
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      // Save phone to DB
      const idToken = await auth.currentUser?.getIdToken();
      await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || "https://justice-pal-cjhn.vercel.app"}/api/lawyers/profile`, {
        method: "PUT",
